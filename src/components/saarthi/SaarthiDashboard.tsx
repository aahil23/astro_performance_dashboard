...
export function SaarthiDashboard({
  ...
  const shouldShowInitialPopup = useMemo(() => {
    ...
    return isPersonalFeedbackNew(
      data.identity.expertId,
      feedback.version,
    );
  }, [
    data.identity.expertId,
    data.personalFeedback,
  ]);

  const [
    showFeedbackPopup,
    setShowFeedbackPopup,
  ] = useState(shouldShowInitialPopup);
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          const position = Number(
            element.dataset.widgetPosition ?? -1,
          );

          if (widgetId) {
            trackWidgetSeen(widgetId, position);
          }
        });
      },
      {
  ...
  const onDashboardClickCapture = useCallback(
    (
      ...
      if (
        ctaTarget ||
        clickable.dataset.analyticsCta === "true"
      ) {
        trackCtaClick({
          label,
  ...
  return (
    ...
    >
      ...
      {showFeedbackPopup &&
      data.personalFeedback ? (
        <PersonalFeedbackPopup
          feedback={data.personalFeedback}
...
function WidgetRenderer({
  ...
  switch (item.id) {
    ...
    case "personal_feedback":
      return data.personalFeedback ? (
        <PersonalFeedbackWidget
          feedback={data.personalFeedback}
        />
      ) : null;
