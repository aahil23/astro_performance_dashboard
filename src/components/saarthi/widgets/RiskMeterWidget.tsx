...
export function RiskMeterWidget({ risk }: Props) {
  ...
  return (
    <WidgetShell
      ...
      subtitle="Your current safety position"
    >
      <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
        <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3">
          <p className={`min-w-0 text-sm font-bold ${getStateClass(state)}`}>
            {formatState(state)}
          </p>
        ...
        <p className="mt-3 whitespace-pre-line text-xs leading-4 text-muted-foreground">
...
function getStateMessage(state: string): string {
  const messages: Record<string, string> = {
    protected:
      "You are above the safety benchmark.\nMaintain your P1 performance today.",
    watch:
