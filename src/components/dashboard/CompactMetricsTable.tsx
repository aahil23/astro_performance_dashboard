...
const getPriority = (key: string) => {
  const priorityMap: Record<string, number> = {
    chat_online_time_l7: 1,
    chat_busy_time_l7: 2,

    audio_online_time_l7: 3,
    audio_busy_time_l7: 4,

    video_online_time_l7: 5,
    video_busy_time_l7: 6,
      ...
      {sortedMetrics.map((m, i) => (
        <div
          ...
          className={`grid grid-cols-[1fr_auto_auto] items-center gap-4 px-4 py-3 ${
            i !== sortedMetrics.length - 1 ? "border-b border-border/60" : ""
          }`}
        >
          <div>
